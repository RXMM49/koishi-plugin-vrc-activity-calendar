    ...
    try {
      ...
      if (url.includes('rlvrc.cn')) {
        try {
          ...
          ctx.logger('vrchat-activity').info(`从 RLVRC 提取到 ${activities.length} 个活动`)
          ...
          ctx.logger('vrchat-activity').warn('解析 RLVRC 活动失败:', err.message)
          return []
        }
      } else {
...
<body>
  ...
  async function updateJapanActivities() {
    try {
      ...
      japanCachedImageBuffer = await generateCalendarImage(japanActivities, japanLastUpdateTime, 'VRChat 日本活动日历').catch((error) => {
        ctx.logger('vrchat-activity').error('生成日本活动日历图片失败:', error.message)
        return null
      })
    } catch (error) {
  ...
  async function updateChinaActivities() {
    try {
      ...
      chinaCachedImageBuffer = await generateCalendarImage(chinaActivities, chinaLastUpdateTime, 'VRChat 中国活动日历').catch((error) => {
        ctx.logger('vrchat-activity').error('生成中国活动日历图片失败:', error.message)
        return null
      })
    } catch (error) {
        ...
        if (japanCachedImageBuffer && config.autoPush.japan.target) {
          for (const bot of ctx.bots) {
            try {
              ...
              ctx.logger('vrchat-activity').warn(`日本活动推送失败: ${e.message}`)
            }
          }
        }
      } catch (error) {
        ctx.logger('vrchat-activity').error('日本活动数据自动更新失败:', error.message)
      }
        ...
        if (chinaCachedImageBuffer && config.autoPush.china.target) {
          for (const bot of ctx.bots) {
            try {
              ...
              ctx.logger('vrchat-activity').warn(`中国活动推送失败: ${e.message}`)
            }
          }
        }
      } catch (error) {
        ctx.logger('vrchat-activity').error('中国活动数据自动更新失败:', error.message)
      }
    }, config.autoPush.china.interval * 60 * 1000)
  }

  updateJapanActivities().catch(error => {
    ctx.logger('vrchat-activity').error('初始日本活动数据加载失败:', error.message)
  })

  updateChinaActivities().catch(error => {
    ctx.logger('vrchat-activity').error('初始中国活动数据加载失败:', error.message)
  })

  ctx.command('vrchat活动刷新', 'vrchat活动日历')
    .alias('活动刷新')
    .alias('vrchat活动')
    .alias('VRChat活动')
  ...
  ctx.command('vrchat活动日本', '获取日本VRChat活动信息')
    .alias('活动日本')
    .action(async ({ session }) => {
      try {
  ...
  ctx.command('vrchat活动中国', '获取中国VRChat活动信息')
    .alias('活动中国')
    .action(async ({ session }) => {
      try {
