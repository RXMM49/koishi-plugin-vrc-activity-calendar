...
exports.schema = Schema.object({
  ...
  scrapeDelay: Schema.number()
    .default(5000)
    .description("网页加载后等待时间(毫秒)"),
  autoPush: Schema.object({
    japan: Schema.object({
      enabled: Schema.boolean()
        .default(false)
        .description("是否启用日本活动自动推送"),
      interval: Schema.number()
        .default(60)
        .description("日本活动自动推送间隔(分钟)"),
      target: Schema.string()
        .default("")
        .description("日本活动推送目标 (格式: 平台:频道ID)"),
    }).description("日本活动推送设置"),
    china: Schema.object({
      enabled: Schema.boolean()
        .default(false)
        .description("是否启用中国活动自动推送"),
      interval: Schema.number()
        .default(60)
        .description("中国活动自动推送间隔(分钟)"),
      target: Schema.string()
        .default("")
        .description("中国活动推送目标 (格式: 平台:频道ID)"),
...
exports.apply = (ctx, config) => {
  ...
  async function fetchFromWebpage(url) {
    ...
    try {
      ...
      ctx.logger("vrchat-activity").info(`正在加载活动页面: ${url}`);
      ...
      if (url.includes("rlvrc.cn")) {
        try {
          ...
          ctx
            .logger("vrchat-activity")
            .info(`从 RLVRC 提取到 ${activities.length} 个活动`);
          if (activities.length > 0) {
            ctx.logger("vrchat-activity").info("示例活动:");
            activities.slice(0, 3).forEach((a, i) => {
              ctx
                .logger("vrchat-activity")
                .info(`#${i + 1} ${a.date} | ${a.title}`);
            });
          }
          return activities;
        } catch (err) {
          ctx
            .logger("vrchat-activity")
            .warn("解析 RLVRC 活动失败:", err.message);
          return [];
        }
      } else {
        ...
        ctx
          .logger("vrchat-activity")
          .info(`发现 ${iframes.length} 个iframe，尝试提取内容`);
        ...
        for (const iframe of iframes) {
          ...
          ctx.logger("vrchat-activity").info(`检查 iframe: ${frameUrl}`);
          if (!frameUrl.includes("calendar.google.com")) continue;

          try {
            await Promise.race([
              frame
                .waitForSelector("[data-eventchip]", { timeout: 20000 })
                .catch(() => null),
              frame
                .waitForSelector(".chip", { timeout: 20000 })
                .catch(() => null),
              frame
                .waitForSelector(".event-summary", { timeout: 20000 })
                .catch(() => null),
            ]);

            ctx
              .logger("vrchat-activity")
              .info("在 Google Calendar iframe 中检测到活动元素");

            const activities = await frame.evaluate(() => {
              const results = [];

              document
                .querySelectorAll("[data-eventchip], .chip")
                .forEach((el) => {
              ...
              document.querySelectorAll("tr.event-summary").forEach((row) => {
                const title =
                  row.querySelector("td.event-summary")?.textContent.trim() ||
                  "未命名活动";
                const time =
                  row.querySelector("td.event-time")?.textContent.trim() ||
                  "时间未知";
            ...
            ctx
              .logger("vrchat-activity")
              .info(`从 iframe 提取到 ${activities.length} 个活动`);
            if (activities.length > 0) {
              ctx.logger("vrchat-activity").info("示例活动:");
              activities.slice(0, 3).forEach((a, i) => {
                ctx
            ...
            ctx
              .logger("vrchat-activity")
              .warn("解析 iframe 活动失败:", err.message);
          }
        }
      }
  ...
  function getCurrentOrNearbyActivities(activities) {
    ...
    function parseTimeRange(range) {
      const dateTimeMatch = range.match(
...
<body>
  <div class="container">
    <div class="calendar-side">
      ...
      <div style="margin-top: 20px; font-size: 12px;">共 ${
        activities.length
      } 个活动</div>
    </div>
    <div class="activities-side">
      ...
      ${limitedActivities
        .map(
          (a) => `
        <div class="activity">
          <div class="activity-date">${a.date} ${
            a.tag ? "(" + a.tag + ")" : ""
          }</div>
      ...
      <div class="update-time">
        最后更新: ${
          lastUpdateTime ? lastUpdateTime.toLocaleString("zh-CN") : "刚刚"
        }
      </div>
    </div>
  </div>
  ...
  async function updateJapanActivities() {
    try {
      ...
      if (newActivities.length > 0) {
        ...
        ctx.logger("vrchat-activity").warn("抓取到 0 个日本活动，保留上次数据");
      }
      ...
      japanCachedImageBuffer = await generateCalendarImage(
        ...
        ctx
          .logger("vrchat-activity")
          .error("生成日本活动日历图片失败:", error.message);
        return null;
      });
    } catch (error) {
      ctx.logger("vrchat-activity").error("更新日本活动失败:", error.message);
    }
  }
  ...
  async function updateChinaActivities() {
    try {
      ...
      if (newActivities.length > 0) {
        ...
        ctx.logger("vrchat-activity").warn("抓取到 0 个中国活动，保留上次数据");
      }
      ...
      chinaCachedImageBuffer = await generateCalendarImage(
        ...
        ctx
          .logger("vrchat-activity")
          .error("生成中国活动日历图片失败:", error.message);
        return null;
      });
    } catch (error) {
      ctx.logger("vrchat-activity").error("更新中国活动失败:", error.message);
    }
  }
  ...
  if (config.autoPush?.japan?.enabled) {
    ctx.setInterval(async () => {
      try {
        ...
        ctx.logger("vrchat-activity").info("日本活动数据自动更新成功");
        if (japanCachedImageBuffer && config.autoPush.japan.target) {
          for (const bot of ctx.bots) {
            try {
              await bot.sendMessage(
                config.autoPush.japan.target,
                segment.image(
                  `data:image/png;base64,${japanCachedImageBuffer.toString(
                    "base64"
                  )}`
                )
              );
        ...
        ctx
          .logger("vrchat-activity")
          .error("日本活动数据自动更新失败:", error.message);
      }
  ...
  if (config.autoPush?.china?.enabled) {
    ctx.setInterval(async () => {
      try {
        ...
        ctx.logger("vrchat-activity").info("中国活动数据自动更新成功");
        if (chinaCachedImageBuffer && config.autoPush.china.target) {
          for (const bot of ctx.bots) {
            try {
              await bot.sendMessage(
                config.autoPush.china.target,
                segment.image(
                  `data:image/png;base64,${chinaCachedImageBuffer.toString(
                    "base64"
                  )}`
                )
              );
        ...
        ctx
          .logger("vrchat-activity")
          .error("中国活动数据自动更新失败:", error.message);
      }
  ...
  updateJapanActivities().catch((error) => {
    ctx
      .logger("vrchat-activity")
      .error("初始日本活动数据加载失败:", error.message);
  });

  updateChinaActivities().catch((error) => {
    ctx
      .logger("vrchat-activity")
      .error("初始中国活动数据加载失败:", error.message);
  });

  ctx
    .command("vrchat活动刷新", "vrchat活动日历")
    .alias("活动刷新")
    .alias("vrchat活动")
    .alias("VRChat活动")
  ...
  ctx
    .command("vrchat活动日本", "获取日本VRChat活动信息")
    .alias("活动日本")
    .action(async ({ session }) => {
      try {
        ...
        if (japanCachedImageBuffer) {
          ...
          ctx.logger("vrchat-activity").warn("日本活动图片缓存为空");
          if (japanActivities && japanActivities.length > 0) {
            ...
            textResult += `最后更新: ${
              japanLastUpdateTime
                ? japanLastUpdateTime.toLocaleString("zh-CN")
                : "刚刚"
            }`;
            return textResult;
          }
        ...
        ctx.logger("vrchat-activity").error("获取日本活动失败:", error.message);
        return "获取日本活动信息失败，请稍后重试";
      }
    });

  ctx
    .command("vrchat活动中国", "获取中国VRChat活动信息")
    .alias("活动中国")
    .action(async ({ session }) => {
      try {
        ...
        if (chinaCachedImageBuffer) {
          ...
          ctx.logger("vrchat-activity").warn("中国活动图片缓存为空");
          if (chinaActivities && chinaActivities.length > 0) {
            ...
            textResult += `最后更新: ${
              chinaLastUpdateTime
                ? chinaLastUpdateTime.toLocaleString("zh-CN")
                : "刚刚"
            }`;
            return textResult;
          }
        ...
        ctx.logger("vrchat-activity").error("获取中国活动失败:", error.message);
